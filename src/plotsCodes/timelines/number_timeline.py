...
def plot_number_timeline(data: TrainStatsData, params: PlotParams):
    print("Plotting number timeline...")

    past_trips = data.get_past_trips()

    fig, ax = dark_figure()
    years = past_trips["Departure (Local)"].dt.year.unique().tolist()
    number_matrix = np.empty((len(years) + 2, MAX_DAYS_PER_YEAR))
    number_matrix[:2, :] = np.nan
    ...
    for ii, year in enumerate(years):
        for day in range(1, MAX_DAYS_PER_YEAR + 1):
            ...
            number_day = past_trips.loc[mask_past].shape[0]
            number_matrix[ii + 2, day - 1] = number_day if number_day > 0 else np.nan
    ...
    sm = cm.ScalarMappable(
        cmap=color_map, norm=plt.Normalize(vmin=0, vmax=np.nanmax(number_matrix))
    )
    sm.set_array([])
