...
END = datetime(2024, 5, 20, 23, 59, 59)
...
def plot_scandinavia_2024_portrait(trips, mapbox_style_token, mapbox_style_id):
    ...
    cbar = fig.colorbar(sm, orientation="horizontal", cax=cax, boundaries=bounds,
                        ticks=np.linspace(0.5, trip_duration_days + 1.5, 5))
    cbar.ax.set_xticklabels(np.arange(1, trip_duration_days + 1, 6))
    ...
    finish_map(fig, ax, "2024_scandinavia_portrait", colorbar=cbar, show=False, logo_position=[0.05, 0.656, 0.4, 0.3])
